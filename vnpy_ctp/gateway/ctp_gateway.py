...
class CtpTdApi(TdApi):
  ...
  def query_margin_ratio(self, req: MarginRequest):
    """ 保证金率查询 """
    # 保证金率查询字典
    margin_ratio_req = {'BrokerID': self.brokerid, 'InvestorID': self.userid, 'InstrumentID': req.symbol,
                        'ExchangeID': req.exchange.value, 'HedgeFlag': THOST_FTDC_HF_Speculation}
    self.reqid += 1
    # 请求查询保证金率
    count = 10
  ...
  def onRspQryTradingAccount(self, data: dict, error: dict, reqid: int, last: bool) -> None:
    ...
    account: AccountData = AccountData(
      ...
      pre_mortgage=data["PreMortgage"],
      pre_credit=data["PreCredit"],
      pre_deposit=data["PreDeposit"],
      pre_balance=data["PreBalance"],
      pre_margin=data["PreMargin"],
      interest_base=data["InterestBase"],
      ...
      withdraw_quota=data["WithdrawQuota"],
      ...
      delivery_margin=data["DeliveryMargin"],
      exchange_delivery_margin=data["ExchangeDeliveryMargin"],
      reserve_balance=data["ReserveBalance"],
      currency_id=data["CurrencyID"],
      spec_product_margin=data["SpecProductMargin"],
      spec_product_frozen_margin=data["SpecProductFrozenMargin"],
      spec_product_commission=data["SpecProductCommission"],
      spec_product_frozen_commission=data["SpecProductFrozenCommission"],
      spec_product_position_profit=data["SpecProductPositionProfit"],
      pre_fund_mortgage_in=data["PreFundMortgageIn"],
      pre_fund_mortgage_out=data["PreFundMortgageOut"],
      fund_mortgage_in=data["FundMortgageIn"],
      fund_mortgage_out=data["FundMortgageOut"],
      fund_mortgage_available=data["FundMortgageAvailable"],
      mortgageable_fund=data["MortgageableFund"],
